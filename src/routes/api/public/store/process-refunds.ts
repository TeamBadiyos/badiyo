// Idempotent store-order refund worker (same pattern as parcels).
...
export const Route = createFileRoute("/api/public/store/process-refunds")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data: rows, error } = await supabaseAdmin
          ...
          .limit(20);
        if (error) {
          console.error("[store-refunds] query failed", error);
        ...
        for (const row of rows ?? []) {
          ...
          try {
            ...
            try {
              ...
              console.error("[store-refunds] refund lookup failed", row.id, listErr);
            }
            ...
            const res = await fetch(
              `https://api.razorpay.com/v1/payments/${encodeURIComponent(row.razorpay_payment_id)}/refund`,
              {
                ...
                body: JSON.stringify({
                  ...
                  speed: "normal",
                  notes: { store_order_id: row.id, reason: row.refund_reason ?? "" },
                }),
              },
            );
            ...
            if (res.ok) {
              ...
              if (alreadyRefunded) {
                console.warn("[store-refunds] already refunded at gateway", row.id);
                await markRefunded(null);
                done++;
                continue;
              }
              console.error("[store-refunds] razorpay refused", row.id, res.status, text);
              const giveUp = attempts >= 5;
              await supabaseAdmin
                .from("merchant_orders")
                .update({
                  refund_status: giveUp ? "failed" : "refund_pending",
                  needs_attention: giveUp,
            ...
            console.error("[store-refunds] refund call failed", row.id, err);
            await supabaseAdmin
              .from("merchant_orders")
              .update({
